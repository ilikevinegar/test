			...
			<view class="uni-product-list" :style='{"padding":"0px 20rpx 24rpx 40rpx","margin":"48rpx 0 0","flexWrap":"wrap","background":"url() no-repeat center top / 100% auto","flex":"1","display":"flex","width":"calc(100% - 240rpx)","justifyContent":"space-between","height":"auto"}'>
				<view @tap="onDetailTap(product)" class="uni-product" :style='{"padding":"20rpx","margin":"0 0 32rpx","borderColor":"#b2e2eb","borderRadius":"24rpx","flexWrap":"wrap","borderWidth":"16rpx","background":"rgba(255,255,255,.9)","display":"flex","width":"100%","position":"relative","borderStyle":"solid","height":"auto"}' v-for="(product,index) in list" :key="index">
					<view :style='{"width":"100%","padding":"8rpx 20rpx","justifyContent":"space-between","display":"flex","height":"auto","order":"5"}'>
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('discussjinrixinwen','修改')) || (!userid && isAuthFront('discussjinrixinwen','修改'))" @click.stop="onUpdateTap(product.id)">
						...
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('discussjinrixinwen','删除')) || (!userid && isAuthFront('discussjinrixinwen','删除'))" @click.stop="onDeleteTap(product.id)">
...
<script>
	export default {
        ...
        params['order'] = 'desc';
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`discussjinrixinwen`, params);
                } else {
                    res = await this.$api.list(`discussjinrixinwen`, params);
                }
				// 如果是第一页数据置空
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('discussjinrixinwen', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search(){
                ...
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`discussjinrixinwen`, searchForm);
                } else {
                    res = await this.$api.list(`discussjinrixinwen`, searchForm);
                }
				// 如果是第一页数据置空
